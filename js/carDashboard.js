var beltInterval = null;
var fuelInterval = null;
var arrowsInterval = null;
var currentValues = {
  "headlights": null,
  "brights": null,
  "arrows": null,
...
function setHeadlights(status){
    if (currentValues["headlights"] == status) {
      return;
    } else {
      currentValues["headlights"] = status;
    }
    if(status)
        $("#headlights").css("opacity", 1);
    else
        $("#headlights").css("opacity", 0.2);
}
function setBrights(status) {
    if (currentValues["brights"] == status) {
      return;
    } else {
      currentValues["brights"] = status;
    }
    if (status)
        $("#brights").css("opacity", 1);
    else
        $("#brights").css("opacity", 0.2);
}
function setArrows(status) {
    if (currentValues["arrows"] == status) {
      return;
    } else {
      currentValues["arrows"] = status;
    }
    if (status){
        arrowsInterval = setInterval(function(){
            var currentOpacity = $("#arrows").css("opacity");
            if(currentOpacity == 1){
                $("#arrows").css("opacity", 0.2);
            }else{
...
function setHood(status) {
    if (currentValues["hood"] == status) {
      return;
    } else {
      currentValues["hood"] = status;
    }
    if (status)
        $("#hood").css("opacity", 1);
    else
        $("#hood").css("opacity", 0.1);
}
function setDoors(status) {
    if (currentValues["doors"] == status) {
      return;
    } else {
      currentValues["doors"] = status;
    }
    if (status)
        $("#doors").css("opacity", 1);
    else
        $("#doors").css("opacity", 0.1);
}
function setTrunk(status) {
    if (currentValues["trunk"] == status) {
      return;
    } else {
      currentValues["trunk"] = status;
    }
    if (status)
        $("#trunk").css("opacity", 1);
    else
        $("#trunk").css("opacity", 0.1);
}
function setEngine(status) {
    if (currentValues["engine"] == status) {
      return;
    } else {
      currentValues["engine"] = status;
    }
    if (status)
        $("#engine").css("opacity", 1);
    else
        $("#engine").css("opacity", 0.2);
}
function setBelt(status) {
    if (currentValues["belt"] == status) {
      return;
    } else {
      currentValues["belt"] = status;
    }
    if (status) {
        beltInterval = setInterval(function () {
            var currentOpacity = $("#belt").css("opacity");
            if (currentOpacity == 1) {
                $("#belt").css("opacity", 0.2);
            } else {
                $("#belt").css("opacity", 1);
            }
        }, 800);
...
function setFuel(value) {
    if(currentValues["fuel"]==value){
      return;
    }else{
      currentValues["fuel"] = value;
    }
    ...
    if (value <= 25) {
        ...
        fuelInterval = setInterval(function () {
            var backgroundColor = (currentBackgroundColor == 0) ? "#5e0e0e" : "#262626";
            currentBackgroundColor = (currentBackgroundColor == 0) ? 1 : 0;
            fm.setBackgroundColor(backgroundColor);
        }, 1000);
        updateFuel(value);
    }else{
        backgroundColor = "#262626";
        if(fuelInterval != null){
            clearInterval(fuelInterval);
            fuelInterval = null;
        }
        fm.setBackgroundColor(backgroundColor);
...
function setPosition(direction, street, city){
    $("#direction").text(direction);
    $("#main-street").text(street);
    $("#city").text(city);
}
function setSpeed(speed){
    $("#speedometer").text(('000' + speed).slice(-3));
}
function setOdometer(Kms) {
    $("#odometer").text(('000000000' + Kms).slice(-9) + " Km");
