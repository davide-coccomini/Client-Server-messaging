...
let selectedFirstColor = undefined;
let selectedSecondColor = undefined;

$(document).ready(function() {
    generaColori();
    $('#first-custom-color').farbtastic(function(color) {
        // Change the vehicle's primary color
        selectedFirstColor = hexToRgb(color).r + ',' + hexToRgb(color).g + ',' + hexToRgb(color).b;
        mp.events.call("repaintVehicle", selectedColorType, selectedFirstColor, selectedSecondColor, -1, 0);
    });

    $('#second-custom-color').farbtastic(function(color) {
        // Change the vehicle's secondary color
        selectedSecondColor = hexToRgb(color).r + ',' + hexToRgb(color).g + ',' + hexToRgb(color).b;
        mp.events.call("repaintVehicle", selectedColorType, selectedFirstColor, selectedSecondColor, -1, 0);
    });
...
function generaColori() {
    var colorRGB = [
        [255, 0, 0 ], [0, 255, 0 ], [0,0,255],[255,117,20],[204,0,153],[77,0,0],[153,51,51],[255,255,0],[0,0,0]
    ];
    for (let index = 0; index < colorRGB.length; index++) {
        const element = colorRGB[index];   
        $("#firtsInputColor").append(generate(element[0], element[1], element[2])+"<br>");
        $("#secondInputColor").append(generate(element[0], element[1], element[2])+"<br>");
    }
}
...
function generate(r, g, b) {
    ...
    var m = 50;
    for (var i = 0; i < 9; i++) {
        str += "<button class='bottoneColore' style='background-color:rgb(" + r + "," + g + "," + b + ");' onclick='changeFirstColorAutomobile(" + r + ", " + g + ", " + b + ")'></button>";
        m = m - 13;
        r += m;
        g += m;
        b += m;
...
function changeFirstColorAutomobile(r, g, b) {
    var rgb = "(" + r + "," + g + "," + b + ")";
    console.log(rgb);
    selectedFirstColor = rgb;
    mp.events.call('repaintVehicle', selectedColorType, selectedFirstColor, selectedSecondColor, -1, 0);
}

function changeSecondColorAutomobile(rgb) {
    console.log(rgb);
    selectedSecondColor = rgb;
    mp.events.call('repaintVehicle', selectedColorType, selectedFirstColor, selectedSecondColor, -1, 0);
}
...
$('.tabs-menu').on('click', 'div', function() {
    ...
    if ($(this).hasClass('active-tab') == false) {
        ...
        selectedColorType = $('.tabs-menu > div').index(this);
        ...
        switch (selectedColorType) {
            case 0:
                ...
                $('#pearlescent-color').removeClass('no-display');
                $('#predefined').removeClass('no-display');

                // Ponemos los colores por defecto
                selectedFirstColor = '0,0,0';
                selectedSecondColor = '0,0,0';
            ...
            case 1:
                ...
                $('#pearlescent-color').addClass('no-display');
                $('#custom').removeClass('no-display');

                // Ponemos los colores por defecto
                selectedFirstColor = 0;
                selectedSecondColor = 0;

                // Quitamos el color perlado
                selectedPearlescentColor = -1;

                break;
        }

        // Repintamos el vehículo
        mp.events.call("repaintVehicle", selectedColorType, selectedFirstColor, selectedSecondColor, selectedPearlescentColor, 0);
...
function acceptRepaint() {
    ...
    if (price > 0) {
        ...
        mp.events.call("repaintVehicle", selectedColorType, selectedFirstColor, selectedSecondColor, selectedPearlescentColor, price);
    }
}

function cancelRepaint() {
    mp.events.call("closeRepaintWindow");
}
