...
// Check if the player name matches a roleplay one
document.getElementById('character-name').addEventListener('keydown', checkRoleplayName);
document.getElementById('character-surname').addEventListener('keydown', checkRoleplayName);

mp.events.add('populateDefaultSkins', (skinJson) => {
	...
	for(let i = 0; i < skinList.length; i++) {
		...
		row.onclick = function() {
			...
			mp.events.call('selectDefaultCharacter', skinList[i]);
		};
...
mp.events.add('showPlayerDuplicatedWarn', () => {
;	$('#error-message').html(i18next.t('character.name-duplicated'));
	$('#error').removeClass('no-display');
});

function checkRoleplayName(event) {
	event = event || window.event;

	// Get the pressed key
	let key = event.keyCode || event.which;

	if (key === 32 || (key >= 48 && key <= 57) || (key >= 96 && key <= 105)) {
...
function toggleCharacterCustomize() {
	...
	if(document.getElementById('character-model-default').checked) {
		...
		mp.events.call('getDefaultSkins');
	} else {
...
$("div#sex").on("click", "img", function() {
	if($(this).hasClass("enabled") == false) {
		...
		mp.events.call('updatePlayerSex', characterSex);
	}
});
...
function createCharacter() {
	if($.trim($('#character-name').val()).length == 0) {
		...
		mp.events.call('acceptCharacterCreation', characterName.trim() + " " + characterSurname.trim(), characterAge);
	}
}

function changeCharacterModel(model) {
	// Change the player's model
	mp.events.call('changePlayerModel', model);
}
...
function changeCharacterSex(sex) {
	if(characterSex !== sex) {
		...
		characterSex = sex;
		mp.events.call('changePlayerSex', sex);
