...
function initializePlayerData(name, age, sex, money, bank, job, rank, extended, clan) {
	...
	if(clan != 'unemployed') {
		var clanDiv = document.getElementById('clan');
		if(clanDiv != undefined)
			clanDiv.classList.remove('invisible')
	}

	// Populate the basic data
	populateBasicData(name, age, sex, money, bank, job, rank, clan);
}

function populateBasicData(name, age, sex, money, bank, job, rank, clan) {
	// Hide all the panels
	hidePanels();
...
mp.events.add('populateBasicData', (name, age, sex, money, bank, job, rank, clan) => {
	// Call the linked function
	populateBasicData(name, age, sex, money, bank, job, rank, clan);
});

mp.events.add('populatePropertiesData', (propertiesJson, rented) => {
	// Hide all the panels
	hidePanels();
...
mp.events.add('populateVehiclesData', (ownedVehiclesJson, lentVehiclesJson) => {
	// Hide all the panels
	hidePanels();
